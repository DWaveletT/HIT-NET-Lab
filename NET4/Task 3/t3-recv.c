...
#define UDP_FWD_IP1  "192.168.1.2"
#define UDP_FWD_IP2  "192.168.2.2"
#define UDP_DST_IP   "192.168.2.1"
...
int main(){
  ...
  recv_addr.sin_port = htons(UDP_DST_PORT2);
  recv_addr.sin_addr.s_addr = inet_addr(UDP_DST_IP);

  // 绑定套接字到本地地址
  if (bind(sockfd_recv, (struct sockaddr *)&recv_addr, sizeof(recv_addr)) < 0) {
    perror("Bind");
    return 1;
  } else {
  ...
  while(1){
    ...
    int recv_len = recvfrom(sockfd_recv, message, sizeof(message), 0, (struct sockaddr *)&send_addr, &addr_len);
    if (recv_len < 0) {
      perror("Recvfrom");
      return 1;
    }
    ...
    iph->id = htonl(UDP_SRC_PORT2);
    iph->frag_off = 0;
    iph->ttl = 255;
    iph->protocol = IPPROTO_UDP;
    iph->check = 0;
    iph->saddr = inet_addr(UDP_DST_IP);
    iph->daddr = inet_addr(UDP_SRC_IP);
    ...
    udph->source = htons(UDP_DST_PORT1);
    udph->dest = htons(UDP_SRC_PORT2);
